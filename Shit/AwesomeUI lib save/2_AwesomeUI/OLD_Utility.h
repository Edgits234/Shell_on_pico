...
class Function
{
    ...
    //constructor for functions that don't have the same template as this Function class
...
class Array
{
  ...
  void add(T input)
  {
    ...
    {
      println(F("ERROR, function 'add()' wasn't able to add an element to the end of the list because the array is full (max array size is : "),(sizeof(array) / sizeof(T)),F(", while current size is : "),size,F(")"));
    }
  }
  ...
  T& at(unsigned int index)
  {
    ...
    }else
    {
      ...
      println(F("ERROR, tried accessing index : "),index,F(" but the range of possible indexs are : "),0,F(" to "),min((size - 1), (sizeof(array) / sizeof(T) - 1)));
    }
