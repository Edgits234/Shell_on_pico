...
class UI 
{
  ...
  void printarray()
  {
    println(F(" this function should not be playing (line "),__LINE__,F(")"));
  ...
  UIelement* findElementWithId(const char* id)
  {
    ...
    println(F("ERROR, COULD NOT FIND THE UI ELEMENT WITH ID:'"),id,F("' in the following list of UIs : "));
  ...
  */
  ...
  void tick()
  {
    println(F("this function should not be called (line : 1009)"));
  ...
  void update()
  {
    ...
    //after the keyboard (bcs emulation is weird and stops the whole code so we don't want to stop code after we drew a window)
      ...
      println(F("drawing the background"));
...
class UIBridge
{
  ...
  Button& addButton(const char* id_input, String text, int fontsize, uint16_t colour, T3 textOffsetX, T4 textOffsetY, bool background, uint16_t backgroundcolor, T1 posX, T2 posY, uint16_t width, uint16_t height, Function<void, Button*, bool> function, bool toggle);//UI::addButton
...
class Window : public UIBridge, public UIelement
{
  ...
  Window(UI* ui1, const char* id_input, uint16_t colour, T1 xpos, T2 ypos, int16_t width, int16_t height)
  {
    ...
    println(F("x = setPos("),xpos,F(", "),w,F(", "),ui1->w,F(", 0);"));
    println(F("y = setPos("),ypos,F(", "),h,F(", "),ui1->h,F(", 1);"));
...
template <typename T1, typename T2, typename T3, typename T4>
Button& UIBridge::addButton(const char* id_input, String text, int fontsize, uint16_t colour, T3 textOffsetX, T4 textOffsetY, bool background, uint16_t backgroundcolor, T1 posX, T2 posY, uint16_t width, uint16_t height, Function<void, Button*, bool> function, bool toggle)
{
...
{
  Button* element = new Button(currUI, id_input, "Button", 1, rgb(50, 50, 0), "middle", "middle", true, rgb(100, 100, 0), "middle", "middle", RELW(200), RELH(100), nullptr, false);
  arr->add((UIelement*)element);
  return *element;
}
// get the desired ui
...
Window& UIBridge::addWindow(const char* id_input, uint16_t colour, T1 xpos, T2 ypos, uint16_t width, uint16_t height)
{
  if(currUI == nullptr)
  {
    ::println(F("ERROR currUI is nullptr"));
    while(true);
  }

  if(arr == nullptr)
  {
    ::println(F("ERROR arr is nullptr"));
    while(true);
  }
